...
Use this file only to test whether the skill router chooses the right file. Do
not load it during normal work.
...
| My Seedance video came back looking weird, how do I diagnose the issue?              | `seedance-cinematic-prompt-director`     | `references/retake-protocol.md`, `references/failure-atlas.md`  |
| How do I animate this still image into a video without it changing the character?    | `seedance-cinematic-prompt-director`     | `references/i2v-guide.md`                                       |
| Create a Seedance prompt that cuts between 3 different angles in one generation.     | `seedance-cinematic-prompt-director`     | `references/multishot-grammar.md`                               |
| I need characters to speak in my video, how do I write the dialogue prompt?          | `seedance-cinematic-prompt-director`     | `references/audio-guide.md`                                     |
| Please rewrite this prompt and remove all the slop words like 'cinematic'.           | `seedance-cinematic-prompt-director`     | `references/anti-slop-lexicon.md`                               |
...
- It loads examples only when the user asks for demonstrations or the task needs
  a pattern.
- It loads model quirks only when the task targets GPT Image 2 or Seedance 2.0
  specifically.
- It loads advanced techniques only when the task needs depth beyond the core
  workflow.
